...
export const usePostWrite = () => {
    ...
    const removeFile = async () => {
        ...
        const newCurrentIndex =
            remainingPreviews.length === 0 ? 0 : current >= remainingPreviews.length ? remainingPreviews.length - 1 : current;
    ...
    const goToPrevious = () => {
        setCurrent((currentIndex) => (previews.length ? (currentIndex - 1 + previews.length) % previews.length : 0));
    };
